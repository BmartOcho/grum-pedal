...
const int LINE_IN_LEVEL = 0;         // MINIMUM GAIN - was 5, now 0 to fix clipping
...
bool detectOnset(float peakLevel, float& velocity) {
    ...
    if (peakLevel >= 0.99) {
        ...
        if (timeOK && pitchConfidence > 0.7 && confDiff > 0.3) {
            velocity = 0.7;  // Default velocity when clipping
            lastOnsetTime = millis();
            Serial.println("CLIPPING MODE: Using confidence-based trigger");
            return true;
        }
        return false;
    }
...
void triggerDrum(int drumIndex, float velocity, float frequency) {
    ...
    Serial.print(" conf:");
    Serial.println(pitchConfidence, 2);
}
...
// ========== MAIN LOOP ==========
void loop() {
    // Read current peak level
    float currentPeak = peak.read();
    
    // Clipping detection and warning
    static unsigned long lastClipWarning = 0;
    if (currentPeak >= 0.99 && millis() - lastClipWarning > 2000) {
        Serial.println("⚠️ INPUT CLIPPING! Reduce line-in level or guitar volume!");
    ...
    if (notefreq.available()) {
        ...
        // Debug pitch even without onset
        static unsigned long lastPitchDebug = 0;
        if (millis() - lastPitchDebug > 1000 && frequency > 0) {
            Serial.print("Pitch detected: ");
            Serial.print(frequency, 1);
            Serial.print(" Hz, confidence: ");
            Serial.println(pitchConfidence, 2);
            lastPitchDebug = millis();
        }
        
        // Only process if we have an onset AND confident pitch
        if (onsetDetected && frequency > 0 && pitchConfidence > 0.6) {  // Lowered from 0.7
            ...
            if (bandIndex >= 0 && canRetrigger(bandIndex)) {
                ...
                Serial.print(frequency);
                Serial.println(" Hz not in any band");
            }
        }
        
        // FALLBACK: If clipping but have good pitch, trigger anyway
        else if (currentPeak >= 0.99 && frequency > 0 && pitchConfidence > 0.8) {
            static unsigned long lastFallbackTrigger = 0;
            if (millis() - lastFallbackTrigger > 100) {  // Simple rate limiting
                int bandIndex = getFrequencyBand(frequency);
                if (bandIndex >= 0) {
                    Serial.print("FALLBACK TRIGGER (clipping): ");
                    triggerDrum(bandIndex, 0.5, frequency);  // Medium velocity
                    lastFallbackTrigger = millis();
                }
            }
        }
    }
    
    // Test mode - trigger on ANY significant change if we're not clipping
    static float lastTestPeak = 0;
    if (currentPeak < 0.95 && currentPeak > 0.05 && currentPeak > lastTestPeak * 1.5) {
        Serial.print("TEST TRIGGER: peak=");
        Serial.print(currentPeak, 3);
        Serial.println(" (onset detection may be too strict)");
    }
    lastTestPeak = currentPeak;
    
    // Periodic debug output
    static unsigned long lastDebugTime = 0;
    if (millis() - lastDebugTime > 500) {
        if (currentPeak > 0.001 && currentPeak < 0.99) {
            Serial.print(".");  // Activity indicator (only if not clipping)
        }
        
        // Detailed debug every 2 seconds
        static int debugCounter = 0;
        if (++debugCounter >= 4) {
            debugCounter = 0;
            Serial.print("\n[DEBUG] peak:");
            Serial.print(currentPeak, 4);
            Serial.print(" env:");
            Serial.print(envelopeEMA, 4);
            Serial.print(" thr:");
            Serial.print(max(NOISE_FLOOR, envelopeEMA * ONSET_THRESHOLD), 4);
            
            if (notefreq.available()) {
                float f = notefreq.read();
                float c = notefreq.probability();
                if (f > 0) {
                    Serial.print(" freq:");
                    Serial.print(f, 1);
                    Serial.print(" conf:");
